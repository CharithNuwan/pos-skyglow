...
export async function GET() {
  try {
    ...
    const categories = await query(
      `SELECT * FROM categories WHERE is_active = 1 AND company_id = ? ORDER BY category_name`,
      [company_id]
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await execute(
      `INSERT INTO categories (category_name, description, is_active, company_id, created_at, updated_at) VALUES (?, ?, 1, ?, datetime('now'), datetime('now'))`,
      [category_name, description || null, company_id]
    );
