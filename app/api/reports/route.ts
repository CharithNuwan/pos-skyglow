...
export async function GET(req: NextRequest) {
  try {
    ...
    const [summary, dailySales, topProducts, topCategories, paymentMethods] = await Promise.all([
      queryOne(`SELECT COUNT(*) as total_orders, COALESCE(SUM(subtotal),0) as total_subtotal, COALESCE(SUM(tax_amount),0) as total_tax, COALESCE(SUM(discount_amount),0) as total_discount, COALESCE(SUM(total_amount),0) as total_revenue, COALESCE(AVG(total_amount),0) as avg_order FROM sales WHERE company_id = ? AND date(sale_date) BETWEEN ? AND ? AND payment_status='completed'`, [company_id, dateFrom, dateTo]),
      query(`SELECT date(sale_date) as date, COUNT(*) as orders, SUM(total_amount) as revenue FROM sales WHERE company_id = ? AND date(sale_date) BETWEEN ? AND ? AND payment_status='completed' GROUP BY date(sale_date) ORDER BY date`, [company_id, dateFrom, dateTo]),
      query(`SELECT p.product_name, SUM(si.quantity) as quantity_sold, SUM(si.total_price) as revenue FROM sale_items si JOIN products p ON si.product_id=p.product_id JOIN sales s ON si.sale_id=s.sale_id WHERE s.company_id = ? AND date(s.sale_date) BETWEEN ? AND ? AND s.payment_status='completed' GROUP BY si.product_id ORDER BY quantity_sold DESC LIMIT 10`, [company_id, dateFrom, dateTo]),
      query(`SELECT c.category_name, SUM(si.quantity) as quantity_sold, SUM(si.total_price) as revenue FROM sale_items si JOIN products p ON si.product_id=p.product_id JOIN categories c ON p.category_id=c.category_id JOIN sales s ON si.sale_id=s.sale_id WHERE s.company_id = ? AND date(s.sale_date) BETWEEN ? AND ? AND s.payment_status='completed' GROUP BY c.category_id ORDER BY revenue DESC LIMIT 5`, [company_id, dateFrom, dateTo]),
      query(`SELECT payment_method, COUNT(*) as count, SUM(total_amount) as amount FROM sales WHERE company_id = ? AND date(sale_date) BETWEEN ? AND ? AND payment_status='completed' GROUP BY payment_method`, [company_id, dateFrom, dateTo]),
    ]);
