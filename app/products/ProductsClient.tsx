  ...
  async function saveProduct() {
    ...
    try {
      ...
      if (data.success || data.product_id) {
        setMsg('Product saved!'); setShowModal(false); loadProducts();
    ...
    <div>
      ...
      <div className="card">
        <div className="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <button className="btn btn-primary btn-sm" onClick={() => { setEditProduct({ selling_price: 0, cost_price: 0, quantity: 0, minimum_stock: 5, is_active: 1 }); setShowModal(true); }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={p.product_id}>
                    ...
                    <td><span className={p.quantity <= p.minimum_stock ? 'text-danger fw-600' : ''}>{p.quantity}{p.quantity <= p.minimum_stock && <span className="low-stock-badge ms-1">Low</span>}</span></td>
                    <td><span className={`badge ${p.is_active ? 'bg-success' : 'bg-secondary'}`}>{p.is_active ? 'Active' : 'Inactive'}</span></td>
                    <td>
                      <button className="btn btn-sm btn-outline-primary me-1" onClick={() => { setEditProduct({ ...p }); setShowModal(true); }}><i className="bi bi-pencil" /></button>
                      <button className="btn btn-sm btn-outline-danger" onClick={() => deleteProduct(p.product_id)}><i className="bi bi-trash" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="row g-3">
                  <div className="col-md-8"><label className="form-label">Product Name *</label><input className="form-control" value={editProduct.product_name || ''} onChange={e => setEditProduct(p => ({ ...p!, product_name: e.target.value }))} /></div>
                  <div className="col-md-4"><label className="form-label">Barcode</label><input className="form-control" value={editProduct.barcode || ''} onChange={e => setEditProduct(p => ({ ...p!, barcode: e.target.value }))} /></div>
                  ...
                  <div className="col-md-4"><label className="form-label">Cost Price</label><input type="number" className="form-control" min="0" step="0.01" value={editProduct.cost_price || 0} onChange={e => setEditProduct(p => ({ ...p!, cost_price: parseFloat(e.target.value) }))} /></div>
                  <div className="col-md-4"><label className="form-label">Selling Price *</label><input type="number" className="form-control" min="0" step="0.01" value={editProduct.selling_price || 0} onChange={e => setEditProduct(p => ({ ...p!, selling_price: parseFloat(e.target.value) }))} /></div>
                  <div className="col-md-4"><label className="form-label">Quantity</label><input type="number" className="form-control" min="0" value={editProduct.quantity || 0} onChange={e => setEditProduct(p => ({ ...p!, quantity: parseInt(e.target.value) }))} /></div>
                  <div className="col-md-4"><label className="form-label">Min Stock</label><input type="number" className="form-control" min="0" value={editProduct.minimum_stock || 5} onChange={e => setEditProduct(p => ({ ...p!, minimum_stock: parseInt(e.target.value) }))} /></div>
                  <div className="col-md-4">
