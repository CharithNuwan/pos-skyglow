          ...
          <div className="row g-3">
                    ...
                    <tbody>
                      {(data?.topProducts || []).length === 0 ? (
                        ...
                        <tr key={p.product_name}>
                          ...
                          <td>{curr}{Number(p.revenue).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="col-lg-6">
              <div className="card">
                ...
                <div className="card-body p-0">
                  <table className="table mb-0">
                    <thead><tr><th>#</th><th>Category</th><th>Sold</th><th>Revenue</th></tr></thead>
                    <tbody>
                      {(data?.topCategories || []).length === 0 ? (
                        ...
                        <tr key={c.category_name}>
                          ...
                          <td>{curr}{Number(c.revenue).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {(data?.paymentMethods || []).map((pm: any) => (
                        <tr key={pm.payment_method}>
                          <td className="text-capitalize fw-500">{pm.payment_method}</td>
                          <td>{pm.count}</td>
                          <td className="fw-600">{curr}{Number(pm.amount).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="col-lg-6">
              <div className="card">
                ...
                <div className="card-body p-0" style={{ maxHeight: 300, overflow: 'auto' }}>
                  <table className="table mb-0">
                    ...
                    <tbody>
                      {(data?.dailySales || []).length === 0 ? (
                        ...
                        <tr key={d.date}>
                          <td>{d.date}</td>
                          <td>{d.orders}</td>
                          <td className="fw-600">{curr}{Number(d.revenue).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
export default function ReportsPage() {
  return <AppLayout title="Reports" requiredRole="manager"><ReportsContent /></AppLayout>;
}
