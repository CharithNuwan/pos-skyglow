            ...
            <div className="row g-3">
              {group.items.map(f => (
                <div key={f.key} className="col-md-6">
                  ...
                  <input type={f.type} className="form-control" value={settings[f.key] || ''} onChange={e => update(f.key, e.target.value)} />
                </div>
              ))}
            </div>
          </div>
...
export default function SettingsPage() {
  return <AppLayout title="Settings" requiredRole="admin"><SettingsContent /></AppLayout>;
}
